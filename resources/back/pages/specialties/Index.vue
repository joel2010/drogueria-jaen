...
const { showModal, record, records } = storeToRefs(specialtiesStore)
...
const onGetRecords = async () => {
    loading.value = true
    await specialtiesStore.onGetRecords().finally(() => loading.value = false)
}
...
const onDeleteRow = (row: any) => {
    ElMessageBox.confirm(
        `¿Estas seguro de eliminar la especialidad seleccionada: ${row.name}?`,
        'Cuidado',
        {
        ...
        .then(async () => {
            loading.value = true
            const response = await specialtiesStore.onDelete(row.id).finally(() => loading.value = false)
            ElNotification({
...
<template>
    <el-card v-loading="loading">
        ...
        <el-table :data="records">
            <el-table-column prop="name" label="Nombre de la especialidad" width="300" />
