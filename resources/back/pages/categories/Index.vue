...
const { showModal, record, records } = storeToRefs(categoriesStore)
...
const onGetRecords = async () => {
    loading.value = true
    await categoriesStore.onGetRecords().finally(() => loading.value = false)
}
...
const onDeleteRow = (row: any) => {
    ElMessageBox.confirm(
        `¿Estas seguro de eliminar la categoría seleccionada: ${row.name}?`,
        'Cuidado',
        {
        ...
        .then(async () => {
            loading.value = true
            const response = await categoriesStore.onDelete(row.id).finally(() => loading.value = false)
            ElNotification({
