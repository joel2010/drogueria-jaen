...
const { showModal, record, records } = storeToRefs(brandsStore)
...
const onGetRecords = async () => {
    loading.value = true
    await brandsStore.onGetRecords().finally(() => loading.value = false)
}
...
const onDeleteRow = (row: any) => {
    ElMessageBox.confirm(
        `¿Estas seguro de eliminar la marca seleccionada: ${row.name}?`,
        'Cuidado',
        {
        ...
        .then(async () => {
            loading.value = true
            const response = await brandsStore.onDelete(row.id).finally(() => loading.value = false)
            ElNotification({
